      ...
      stage('Deploy to DEV') {
        steps {
             script {
                   ...
                   openshift.withProject(env.DEV_PROJECT) {
                     sh """
                        sed -i -e 's#image-registry.openshift-image-registry.svc:5000/demo/sessions-twas:1.0#${env.DEV_IMAGE_TAG}#' session-mgmt/yaml/dc.yaml
                        """
